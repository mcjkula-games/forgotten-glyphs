...
func get_scores_by_player(player_name, maximum=10, ldboard_name="main", period_offset=0):
	...
	var game_id = SilentWolf.config.game_id
	var game_version = SilentWolf.config.game_version
	var request_url = "https://api.silentwolf.com/get_scores_by_player/" + str(game_id) + "?version=" + str(game_version) + "&max=" + str(maximum)  + "&ldboard_name=" + str(ldboard_name) + "&player_name=" + str(player_name) + "&period_offset=" + str(period_offset)
